...
struct Params {
  ...
  zn : u32,     // [ZFUSE 2026-09-11] 本 dispatch 里一个线程要跑的 z 片数;0 视作 1 = 旧行为
};
...
// [COMPACT 2026-09-14] 压缩刀:detect 拆两段。
//   A 段(本文件)= 每线程 2 载入 + 0.8t 门 + 26 邻居测试;过了就把 (x0,y0,zc) 压缩追加进候选表。
//   B 段(sift_dog_refine.wgsl)= 每线程一个候选,密集跑精化+打分+发射。
// 动机(A16 实测):极值点只占线程的 0.00115%,32-lane 组激活率 3.6%、有效 lane 0.115%
//   ⇒ 那一段的发散浪费约 31×(24 ms 里有用功只值 ~0.8 ms)。
// 比较与取值一个不变,只改「谁干哪份活」;原子追加顺序变,而它本来就是非确定的。
@group(0) @binding(5) var<storage, read_write> cand_buf : array<u32>;   // ci*4: x0,y0,zc,pad
@group(0) @binding(6) var<storage, read_write> cand_counter : atomic<u32>;
...
// [ZFUSE 2026-09-11] 原 main 的**函数体逐字未动**,只是抽成一个可被多次调用的
// 函数,好让一个线程依次处理相邻的几个 zc。抽出来的是语句,不是数学:
// v 由调用方算好传进来,值与原来的 dog_at(zc,x0,y0) 逐位相同。
// [COMPACT-B] 每线程一个候选:密集,无发散。下面的精化+打分+发射**逐字未动**。
@compute @workgroup_size(64, 1, 1)
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
  let i : u32 = gid.x;
  if (i >= atomicLoad(&cand_counter) || i >= P.cap) { return; }
  let x0 : i32 = i32(cand_buf[i * 4u + 0u]);
  let y0 : i32 = i32(cand_buf[i * 4u + 1u]);
  let zc : i32 = i32(cand_buf[i * 4u + 2u]);
  let W  : i32 = i32(P.width);
  let H  : i32 = i32(P.height);
