...
// Sources: aether_cpp/shaders/wgsl/_brush_raw/prefix_sum_scan_sums.wgsl
...
fn main(@builtin(global_invocation_id) id: vec3<u32>, @builtin(local_invocation_index) gid: u32) {
    ...
    let idx = ((id.x * THREADS_PER_GROUP) - 1u);
    if ((idx >= 0u) && (idx < arrayLength((&input)))) {
        let _e15 = input[idx];
        x = _e15;
    }
    let _e18 = x;
    groupScan(id.x, gid, _e18);
    return;
}
