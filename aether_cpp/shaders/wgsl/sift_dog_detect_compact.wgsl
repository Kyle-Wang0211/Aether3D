...
@group(0) @binding(2) var<storage, read> packed_gss_hi : array<f32>;
// [COMPACT-PZ 2026-09-14] 压缩刀建在 **pre-ZFUSE** 核上(现役核的 ZFUSE 重构即使关着
//   也要 +24.1%,见 09-14 台架三臂)。绑定按实际使用编号 0..4:A 段不发关键点,
//   kp_counter/kp_buffer 是未使用绑定、会被 WGSL 自动布局剔掉。
@group(0) @binding(3) var<storage, read_write> cand_buf : array<u32>;   // ci*4: x0,y0,zc,pad
@group(0) @binding(4) var<storage, read_write> cand_counter : atomic<u32>;
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
  ...
  // [COMPACT-A] 这里不精化,只把候选压缩追加进表(B 段密集消费)。
  let ci : u32 = atomicAdd(&cand_counter, 1u);
  if (ci < P.cap) {
    cand_buf[ci * 4u + 0u] = u32(x0);
    cand_buf[ci * 4u + 1u] = u32(y0);
    cand_buf[ci * 4u + 2u] = u32(zc);
    cand_buf[ci * 4u + 3u] = 0u;
  }
  return;
}
