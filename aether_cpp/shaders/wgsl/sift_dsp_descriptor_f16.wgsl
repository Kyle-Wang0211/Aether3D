// sift_dsp_descriptor_f16.wgsl — f16 DSP-SIFT descriptor variant (speed bet).
//
// Clone of sift_dsp_descriptor.wgsl with the patch (wpatch) and the histogram
// accumulation in f16 — ~2× the f32 ALU/bandwidth on Apple GPUs. The GSS is read
// as f32 (the warp's bilinear taps stay f32 for accuracy), cast to f16 for the
// resident patch; gradient + trilinear scatter + the 128-bin reduction run in
// f16; the final L2/0.2-truncate/L2 normalize is back in f32 (cheap, lane-0).
// The downstream CPU L1RootNormalize + round(512) u8 quantization can absorb
// small f16 round-off — the dsp_descriptor_parity cosine gate (≥0.998) decides
// whether this variant is shippable. The f32 kernel remains the default;
// Adreno (f16-crash history) uses f32.
//
enable f16;
//
