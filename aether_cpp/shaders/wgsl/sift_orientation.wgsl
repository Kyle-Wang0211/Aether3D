// sift_orientation.wgsl — per-keypoint dominant orientation(s) (Stage D).
//
// GPU port of VLFeat vl_covdet_extract_orientations_for_frame (covdet.c:2698)
// + the 1→K feature expansion (covdet.c:2867-2890). One WORKGROUP per input
// keypoint; 64 lanes cooperate on the 41x41 orientation patch + histogram.
//
// Pipeline (covdet.c:2744-2846):
//   A = U D V'   (vl_svd2);  warp uses A_ud = U D ;  theta0 = atan2(V[1],V[0]).
//   patch-helper warp (cross-octave, sigmaD=1) → 41x41 patch.
//   vl_imsmooth_f with anisotropic deltaSigma1/2 over stephat (ALWAYS, the if(1)
//     block — unlike affine where ACCURATE_SMOOTHING is false).
//   polar gradient (modulus, angle) → 36-bin histogram weighted by aaMask, with
//     LINEAR interpolation between the two adjacent bins.
//   6x box-blur smoothing of the circular histogram.
//   peaks: h0 > 0.8*max && h0 > neighbours → parabolic interp → theta = binExtent
//     *(i+di)+theta0. up to MAX_NUM_ORIENTATIONS=4 (sorted by score desc).
//   each orientation rotates the AFFINE ellipse A by R(theta) and is appended as
//     a separate oriented feature (1→K expansion).
//
// Constants (covdet.c:1432-1443):
//   NUM_BINS 36, PEAK_RELATIVE 0.8, MAX_ORIENTATIONS 4
//   PATCH_RESOLUTION 20 → side 41, EXTENT 9, sigmaD 1
//
// f32 throughout (no f64 in WGSL); gate is median orientation err <= 1 deg.
// 16KB workgroup budget: wpatch(6724B) + wsmooth(6724B) + hist(144B) + scratch
// = ~13.6KB < 16KB. The mask is the closed-form gaussian (recomputed per pixel),
// not a fourth resident array — same trick as Stage C.
...
var<workgroup> red     : array<f32, 64>;        // reduction scratch for histogram bins
...
fn main(@builtin(workgroup_id) wid : vec3<u32>,
  ...
  // accumulate per-lane partial histograms into `red`-tiled bins is complex;
  // simplest correct path: each lane atomically-free adds to a PRIVATE 36-bin
  // array then we reduce. 36 f32 per lane * 64 = too big for workgroup mem.
  // Instead: serialize the bin add via a workgroup loop over bins is also slow.
  // Use atomic adds on an integer fixed-point histogram? f32 atomics unavailable.
  // Pragmatic: each lane accumulates into 36 PRIVATE registers (function-local
  // array), then bins are summed across lanes via the `red` reduction, one bin
  // at a time (36 reductions). 36 * log2(64) barriers — fine for a per-kp pass.
  var local_hist : array<f32, 36>;
  for (var b = 0u; b < NUM_BINS; b = b + 1u) { local_hist[b] = 0.0; }
  for (var idx = lane; idx < NPIX; idx = idx + WG) {
    let yy = i32(idx / SIDE); let xx = i32(idx % SIDE);
    let c = wpatch[idx];
    ...
    local_hist[b0] = local_hist[b0] + w1 * (modulus * weight);
    local_hist[b1] = local_hist[b1] + w2 * (modulus * weight);
  }
  // reduce each bin across lanes via `red`.
  for (var b = 0u; b < NUM_BINS; b = b + 1u) {
    red[lane] = local_hist[b];
    workgroupBarrier();
    var off = WG / 2u;
    loop { if (off == 0u) { break; } if (lane < off) { red[lane] = red[lane] + red[lane + off]; } workgroupBarrier(); off = off / 2u; }
    if (lane == 0u) { hist[b] = red[0]; }
    workgroupBarrier();
  }
