...
struct Params {
  ...
  dsp_min_scale  : f32,
  dsp_scale_step : f32,
  groups_x : u32,   // 2D dispatch width (workgroups exceed the 65535 1D limit)
  pad1 : u32,
};

// ── ③ scale-parallel descriptor ──
...
@group(0) @binding(3) var<storage, read_write> scale_desc : array<f32>;   // count*DSP_NUM*128
...
fn main(@builtin(workgroup_id) wid:vec3<u32>,
  ...
  // 2D grid → flat global workgroup index (kp*DSP_NUM + sc). The 1D count
  // (count*DSP_NUM ≈ 119k) exceeds the 65535 per-dim limit, so the host
  // dispatches (groups_x, ceil(total/groups_x)) and we re-flatten here.
  let gw = wid.y * P.groups_x + wid.x;
  let kp = gw / DSP_NUM;
  let sc = gw % DSP_NUM;
  let lane=lid.x;
  ...
  // ── this workgroup computes exactly ONE DSP scale (sc) ──
  {
    ...
    workgroupBarrier();
    // write THIS scale's normalized 128-bin descriptor to scale_desc; the mean
    // pass averages the DSP_NUM scales per kp in fixed order (bit-identical to
    // the serial accumulate).
    let outb = (kp * DSP_NUM + sc) * 128u;
    for(var b=lane;b<128u;b=b+WG){ scale_desc[outb + b] = hist[b]; }
  }
}
