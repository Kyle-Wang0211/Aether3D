// sift_dsp_descriptor_f16_atomic.wgsl — 定点共享原子加直方图(CudaSift 结构)。
//
// [DESC-ATOMIC 2026-08-10 用户签 cosine>=0.998 质量门] 与 _f16 版唯一差异:
// 每 lane 私有 lh[128](512B 寄存器,占用率杀手)+ 28-barrier 归约树,换成
// 128 格共享内存 atomic<i32> 黑板 —— CudaSift ExtractSiftDescriptors 同构
// (CUDA 有 float 原子加;WGSL 没有,用业界标准定点补法:weight*2^19 取整
// 原子加,读回除回)。上限账:bin 和 < 961*1.4≈1345,*2^19=7.0e8 < i32 上限
// 2.1e9(3x 余量)。舍入 2e-6/笔 + 加序改变 ⇒ 非逐位;质量门=f16 同款
// cosine>=0.998(dsp_descriptor_parity),f16 先例实测 0.99998。
// warp/梯度/scatter 权重数学与 _f16 版逐字符相同。
...
var<workgroup> wpatch : array<f16, 961>;       // resident warped patch (f16)
...
const BLK : u32 = 32u;
var<workgroup> histA : array<atomic<i32>, 128>; // 定点原子黑板(自适应系数)
// [DESC-ATOMIC] 每 (kp,scale) 的梯度幅值平方最大值(f32 非负 ⇒ bit 单调,
// 可用 u32 atomicMax)。定点系数按它设"曝光":FXP=2^30/(Mmax*256),
// 256=单 bin 质量上界(win*trilinear 对一个 bin 的合计 < 4*SBP^2≈164)。
// 大 bin 顶不破 i32,小权重精度随 patch 对比度自适应 —— 固定系数在
// 2^19 丢小权重精度(recall 0.928→0.733 实测)、2^23 爆大 bin,两头顶死。
var<workgroup> maxg2_bits : atomic<u32>;
...
fn main(@builtin(workgroup_id) wid:vec3<u32>,
  ...
  for(var sc=0u; sc<DSP_NUM; sc=sc+1u){
    ...
    for(var idx=lane; idx<NPIX; idx=idx+WG){
      ...
      // bilinear in f32 (accuracy), store the patch in f16.
      wpatch[idx]=f16((1.0-wy)*((1.0-wx)*i00+wx*i10)+wy*((1.0-wx)*i01+wx*i11));
    }
    workgroupBarrier();
    ...
    for(var b=lane;b<128u;b=b+WG){ atomicStore(&histA[b], 0); }
    ...
    // [DESC-ATOMIC] 预扫:定 FXP 曝光(梯度公式与下方 scatter 逐字符同)。
    var g2max : f32 = 0.0;
    for(var t=lane; t<nwin; t=t+WG){
      ...
      let c = f32(wpatch[u32(py)*SIDE + u32(px)]);
      var gx:f32; var gy:f32;
      if(px==0){gx=f32(wpatch[u32(py)*SIDE+u32(px+1)])-c;}
      else if(px==i32(SIDE)-1){gx=c-f32(wpatch[u32(py)*SIDE+u32(px-1)]);}
      else{gx=0.5*(f32(wpatch[u32(py)*SIDE+u32(px+1)])-f32(wpatch[u32(py)*SIDE+u32(px-1)]));}
      if(py==0){gy=f32(wpatch[u32(py+1)*SIDE+u32(px)])-c;}
      else if(py==i32(SIDE)-1){gy=c-f32(wpatch[u32(py-1)*SIDE+u32(px)]);}
      else{gy=0.5*(f32(wpatch[u32(py+1)*SIDE+u32(px)])-f32(wpatch[u32(py-1)*SIDE+u32(px)]));}
    ...
    var FXP : f32 = 0.0;
    if(Mmax > 0.0){ FXP = 1073741824.0/(Mmax*256.0); }  // 2^30/(M*256)
    ...
    for(var t=lane; t<nwin; t=t+WG){
      ...
      let dxi = dlo_x + i32(t % wcols);
      let px = xc+dxi; let py = yc+dyi;
      // read the f16 patch into f32 for the gradient/scatter math (transcendentals
      // stay f32; the f16 win is the patch storage/bandwidth + bilinear).
      let c = f32(wpatch[u32(py)*SIDE + u32(px)]);
      var gx:f32; var gy:f32;
      if(px==0){gx=f32(wpatch[u32(py)*SIDE+u32(px+1)])-c;}
      else if(px==i32(SIDE)-1){gx=c-f32(wpatch[u32(py)*SIDE+u32(px-1)]);}
        ...
        for(var dby=0;dby<2;dby=dby+1){
          for(var dbt=0;dbt<2;dbt=dbt+1){
            if(binx+dbx >= -(NBP/2) && binx+dbx < (NBP/2) &&
              ...
              atomicAdd(&histA[bi], i32(round(weight*FXP)));
            }
          }
        }
      }
    }
    // 黑板读回:定点 → f32(替代原 28-barrier 归约树)。FXP==0(全平 patch)
    // ⇒ hist 置 0,与原路径(全部 modulus<=0 跳过)同语义。
    workgroupBarrier();
    if(FXP > 0.0){
      for(var b=lane;b<128u;b=b+WG){ hist[b]=f32(atomicLoad(&histA[b]))/FXP; }
    } else {
      for(var b=lane;b<128u;b=b+WG){ hist[b]=0.0; }
    }
    workgroupBarrier();
