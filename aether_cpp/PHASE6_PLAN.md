...
- **Final iOS scene renderer port 2026-04-27 (commit pending; user iPhone gate next)** — iOS Runner no longer uses the Phase 5 triangle renderer. `ios/Runner/MetalRenderer.swift` is now a thin FlutterTexture bridge around the shared C++ scene IOSurface renderer (`aether_scene_renderer_create/load_glb/render_full/destroy` via static Swift C ABI references); `AetherTexturePlugin.swift` gained the macOS-equivalent `loadGlb` + `setMatrices` MethodChannel handlers and displayLink now renders the C++ scene per frame. `DamagedHelmet.glb` is bundled under `pocketworld_flutter/assets/models/` and Dart materializes it to a temp file before calling native `loadGlb`, so iPhone devices no longer depend on repo-local `aether_cpp/build/test_assets` paths. iOS device FFI builds with Dawn ON through a viewer-only CMake path (scene renderer + DawnGPUDevice + GLB loader + baked WGSL only), avoiding the old `aether3d_core` CoreML pipeline availability issue; Dawn/Tint/Abseil device archives are merged into `dist/libs/ios-arm64/libaether3d_ffi.a` for CocoaPods. Simulator remains Dawn OFF due the known Abseil iphonesimulator C++17 probe bug, but now carries loud no-op scene symbols so sim links instead of missing-symbol failing. Verification: iPhoneOS `aether3d_ffi` archive exports scene symbols ✅, `flutter build ios --debug --no-codesign` ✅, `cmake --build .` ✅, full native smoke `PASS=29 FAIL=0` ✅, `flutter analyze` ✅, `flutter test` ✅, `flutter build macos --debug` ✅. Visual gate pending: run on Kyle's iPhone 14 Pro and confirm DamagedHelmet appears + gestures work.
- **Phase 6.4x portability boundary 2026-04-27 (commit `42561cd6`)** — Render policy / DRS strategy moved out of long-lived Swift algorithm code into a shared C++ policy module with C ABI (`include/aether/pocketworld/render_policy.h`, `src/pocketworld/render_policy.cpp`). macOS Swift still probes Apple platform facts (hardware model, OS version, EDR headroom, screen size) but now calls `aether_render_policy_choose` for tier/surface/MetalFX/EDR/DRS decisions and wraps `AetherDrsController` through `aether_drs_*` instead of carrying a duplicate Swift DRS implementation. The policy intentionally keeps production DRS staged/off for the current 256×256 Batch 2 texture path; the reusable DRS algorithm is wired and covered by `aether_render_policy_smoke`, while Phase 6.4d.3 remains the trigger to enable it for larger render/display split targets. Verification: `aether_render_policy_smoke` ✅, `cmake --build .` ✅, full native smoke `PASS=29 FAIL=0` ✅, exported policy/DRS symbols present in `libaether3d_ffi.dylib` ✅, `flutter build macos --debug` ✅, `flutter test` ✅.
