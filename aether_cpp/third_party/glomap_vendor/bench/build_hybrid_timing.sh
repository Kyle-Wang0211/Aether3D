#!/usr/bin/env bash
# build_hybrid_timing.sh — standalone build of the S1 hybrid GPU->CPU DSP-SIFT
# extractor + e2e timing harness (bench/extract_hybrid_timing.cc).
#
# Same worktree-only fast path as build_gpuparity.sh: links the PREBUILT host
# Dawn archive (no Dawn rebuild) + the VLFeat .c subset the full extractor needs
# (adds sift.c for the descriptor + the covdet refine/extrema/patch symbols).
# Run from the aether_cpp/ root. See glomap_vendor/GPU_DSP_SIFT_PLAN.md S1.
set -euo pipefail
...
OBJ="${OBJ:-/tmp/hybrid_timing_obj}"
mkdir -p "$OBJ"

VLDEFS="-DVL_DISABLE_SSE2 -DVL_DISABLE_AVX -DVL_DISABLE_OPENMP"

# VLFeat (arm64 scalar) — the units the full hybrid extractor needs:
#   gss: generic host random mathop imopv scalespace covdet
#   descriptor: sift
...
# Hybrid timing harness TU.
c++ -std=c++20 -arch arm64 -O2 \
  -I "$ROOT/tools" -I "$DAWN_INC_SRC" -I "$DAWN_INC_GEN" \
  -I "$VLF" -I "$ROOT/third_party/stb" \
  -c "$ROOT/third_party/glomap_vendor/bench/extract_hybrid_timing.cc" \
  -o "$OBJ/extract_hybrid_timing.o"
...
c++ -std=c++20 -arch arm64 -g -Wl,-search_paths_first -Wl,-headerpad_max_install_names \
  "$OBJ/extract_hybrid_timing.o" "$OBJ/dawn_kernel_harness.o" \
  ...
  -o "$OBJ/extract_hybrid_timing_exe"

echo "built: $OBJ/extract_hybrid_timing_exe"
echo "run (from $ROOT):"
echo "  $OBJ/extract_hybrid_timing_exe \\"
echo "    third_party/glomap_vendor/iosapp/Resources/sift_test.jpg [num_threads]"
