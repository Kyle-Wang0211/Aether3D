...
// ════════════════════════════════════════════════════════════════════════════
//  PROFILING INSTRUMENTATION (S1 wall-clock attribution).
//  Attributes the per-frame wall-clock to: (a) GPU dispatch Submit+wait (the
//  harness blocks on WaitAny until GPU idle after EVERY dispatch), (b) the
//  copy_to_staging Submit+wait (the GPU->staging copy before a map), (c) the
//  MapAsync spin-wait of readback(), (d) buffer upload (WriteBuffer + create),
//  (e) buffer alloc, (f) pipeline compile (load_compute / Tint). The remainder
//  of the frame wall-clock = pure host CPU glue (sort/compaction/OFrame build/
//  WarpIn build/parity prep). Counts the Submit+wait points and the readbacks.
// ════════════════════════════════════════════════════════════════════════════
struct Prof {
  // accumulated ms per category (reset per frame)
  double dispatch_submit_wait = 0;   // harness.dispatch(): Submit + WaitAny(idle)
  double copy_submit_wait = 0;       // copy_to_staging(): Submit + WaitAny(idle)
  double readback_map_wait = 0;      // readback(): MapAsync spin-wait + memcpy
  double upload_ms = 0;              // upload(): CreateBuffer + WriteBuffer
  double alloc_ms = 0;              // alloc() + alloc_staging_for_readback()
  double pipeline_ms = 0;           // load_compute(): Tint compile + pipeline
  // counts (reset per frame)
  int n_dispatch = 0;               // Submit+wait points from dispatch()
  int n_copy = 0;                   // Submit+wait points from copy_to_staging()
  int n_readback = 0;               // MapAsync readbacks
  int n_upload = 0, n_alloc = 0, n_pipeline = 0;
  // raw Submit (NO per-call wait) used in gss build CopyBufferToBuffer + the
  // detect/refine dispatch_offset path. These are fire-and-forget; their GPU
  // cost is absorbed into the NEXT wait (a copy_to_staging or readback). We time
  // the host-side encode+Submit CPU cost only.
  double raw_submit_ms = 0;
  int n_raw_submit = 0;
  void reset() { *this = Prof{}; }
};
Prof g_prof;

// Thin wrapper over DawnKernelHarness: forwards every method but TIMES the four
// that block the host (dispatch / copy_to_staging / readback) + the resource
// creators (upload / alloc / load_compute). Cheap accessors pass through
// untimed. Drop-in: call sites use `ph.` instead of `harness.`.
struct ProfHarness {
  aether::tools::DawnKernelHarness& h;
  explicit ProfHarness(aether::tools::DawnKernelHarness& hh) : h(hh) {}

  bool init() { return h.init(); }
  const wgpu::Device& device() const { return h.device(); }
  const wgpu::Queue& queue() const { return h.queue(); }
  const wgpu::Instance& instance() const { return h.instance(); }

  wgpu::Buffer upload(const void* data, size_t size, wgpu::BufferUsage usage) {
    auto t0 = clock_t_::now();
    wgpu::Buffer b = h.upload(data, size, usage);
    g_prof.upload_ms += ms_since(t0); ++g_prof.n_upload;
    return b;
  }
  wgpu::Buffer alloc(size_t size, wgpu::BufferUsage usage) {
    auto t0 = clock_t_::now();
    wgpu::Buffer b = h.alloc(size, usage);
    g_prof.alloc_ms += ms_since(t0); ++g_prof.n_alloc;
    return b;
  }
  wgpu::Buffer alloc_staging_for_readback(size_t size) {
    auto t0 = clock_t_::now();
    wgpu::Buffer b = h.alloc_staging_for_readback(size);
    g_prof.alloc_ms += ms_since(t0); ++g_prof.n_alloc;
    return b;
  }
  wgpu::ComputePipeline load_compute(std::string_view src, const char* ep = "main") {
    auto t0 = clock_t_::now();
    wgpu::ComputePipeline p = h.load_compute(src, ep);
    g_prof.pipeline_ms += ms_since(t0); ++g_prof.n_pipeline;
    return p;
  }
  void dispatch(const wgpu::ComputePipeline& p, const std::vector<wgpu::Buffer>& b,
                uint32_t gx, uint32_t gy = 1, uint32_t gz = 1) {
    auto t0 = clock_t_::now();
    h.dispatch(p, b, gx, gy, gz);
    g_prof.dispatch_submit_wait += ms_since(t0); ++g_prof.n_dispatch;
  }
  void copy_to_staging(const wgpu::Buffer& src, const wgpu::Buffer& dst, size_t sz) {
    auto t0 = clock_t_::now();
    h.copy_to_staging(src, dst, sz);
    g_prof.copy_submit_wait += ms_since(t0); ++g_prof.n_copy;
  }
  std::vector<uint8_t> readback(const wgpu::Buffer& buf, size_t size) {
    auto t0 = clock_t_::now();
    std::vector<uint8_t> v = h.readback(buf, size);
    g_prof.readback_map_wait += ms_since(t0); ++g_prof.n_readback;
    return v;
  }
};
...
struct GpuTimings {
  ...
  double total_ms = 0;
  // ── S1 PROFILING breakdown (full-frame attribution of total_ms) ──
  Prof prof{};                  // whole-frame profiling accumulators (snapshot)
  double gss_build_ms = 0;      // gss-build bucket (whole)
  double gss_dispatch_ms = 0, gss_copy_ms = 0, gss_readback_ms = 0;
  double gss_upload_ms = 0, gss_alloc_ms = 0, gss_pipeline_ms = 0;
...
wgpu::Buffer build_resident_gss(
    ProfHarness& h, VlScaleSpace* gss, const Geom& G,
...
int main(int argc, char** argv) {
  ...
  ProfHarness ph(harness);  // S1 profiling shim: routes timed harness calls.
  ...
  for (const std::string& imgpath : imgs) {
    ...
    // ════════════════════════════════════════════════════════════════════════
    GpuTimings T{};
    g_prof.reset();                 // S1: per-frame profiling reset
    ...
    Prof gss_snap = g_prof;  // snapshot to isolate gss-build's GPU categories
    wgpu::Buffer pyramid = build_resident_gss(
        ph, gss, G, &octinfo, &total_floats, &gss_build_ms, blur_wgsl,
        resample_wgsl);
    // gss-build's contribution to each profiling category (delta).
    T.gss_dispatch_ms = g_prof.dispatch_submit_wait - gss_snap.dispatch_submit_wait;
    T.gss_copy_ms = g_prof.copy_submit_wait - gss_snap.copy_submit_wait;
    T.gss_readback_ms = g_prof.readback_map_wait - gss_snap.readback_map_wait;
    T.gss_upload_ms = g_prof.upload_ms - gss_snap.upload_ms;
    T.gss_alloc_ms = g_prof.alloc_ms - gss_snap.alloc_ms;
    T.gss_pipeline_ms = g_prof.pipeline_ms - gss_snap.pipeline_ms;
    T.gss_build_ms = gss_build_ms;
    ...
    const wgpu::Device& dev = ph.device();
    const wgpu::Queue& q = ph.queue();

    // ── B.2 + B.3  detect + refine per octave (resident gss SUB-SLICE) ──
    auto pipe_detect = ph.load_compute(dog_wgsl, "detect");
    auto pipe_refine = ph.load_compute(refine_wgsl, "refine");
    ...
    for (const OctInfo& r : octinfo) {
      ...
      wgpu::Buffer dp_buf = ph.upload(&dp, sizeof(dp), wgpu::BufferUsage::Uniform);
      ...
      auto dispatch_offset = [&](wgpu::ComputePipeline& pipe,
        ...
        wgpu::CommandBuffer cb = enc.Finish();
        q.Submit(1, &cb);
        g_prof.raw_submit_ms += ms_since(t_rs); ++g_prof.n_raw_submit;
      };
      ...
      wgpu::Buffer cc_stage = ph.alloc_staging_for_readback(sizeof(uint32_t));
      ph.copy_to_staging(cand_count, cc_stage, sizeof(uint32_t));
      std::vector<uint8_t> ccb = ph.readback(cc_stage, sizeof(uint32_t));
      ...
      if (cread > 0) {
        ...
        wgpu::Buffer rp_buf = ph.upload(&rp, sizeof(rp), wgpu::BufferUsage::Uniform);
    ...
    uint32_t kp_n = 0;
    {
      wgpu::Buffer s = ph.alloc_staging_for_readback(sizeof(uint32_t));
      ph.copy_to_staging(kp_count, s, sizeof(uint32_t));
      std::vector<uint8_t> b = ph.readback(s, sizeof(uint32_t));
    ...
    std::vector<Keypoint> refined(kp_read);
    {
      ...
      wgpu::Buffer s = ph.alloc_staging_for_readback(bytes);
      ph.copy_to_staging(kp_out, s, bytes);
      std::vector<uint8_t> b = ph.readback(s, bytes);
    ...
    auto pipe_bc = ph.load_compute(suppress_wgsl, "bin_count");
    auto pipe_bs = ph.load_compute(suppress_wgsl, "bin_scatter");
    auto pipe_su = ph.load_compute(suppress_wgsl, "suppress");
    std::vector<uint32_t> keep((size_t)kp_read, 1u);
    {
      ...
      wgpu::Buffer kps_buf = ph.upload(gpu_kp.data(), kp_bytes, wgpu::BufferUsage::Storage);
      wgpu::Buffer P_buf = ph.upload(&P, sizeof(P), wgpu::BufferUsage::Uniform);
      std::vector<uint32_t> zc(num_cells, 0u);
      const size_t cc_bytes = (size_t)num_cells * sizeof(uint32_t);
      wgpu::Buffer cellcnt = ph.upload(zc.data(), cc_bytes,
          wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc | wgpu::BufferUsage::CopyDst);
      std::vector<uint32_t> cstart(num_cells + 1u, 0u);
      wgpu::Buffer cellstart = ph.upload(cstart.data(), cstart.size() * sizeof(uint32_t),
          wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopyDst);
      std::vector<uint32_t> items((size_t)n, 0xFFFFFFFFu);
      wgpu::Buffer items_buf = ph.upload(items.data(), items.size() * sizeof(uint32_t),
          wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc);
      std::vector<uint32_t> keep0((size_t)n, 0u);
      wgpu::Buffer keep_buf = ph.upload(keep0.data(), keep0.size() * sizeof(uint32_t),
          wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc | wgpu::BufferUsage::CopyDst);
      std::vector<uint32_t> alive0((size_t)n, 1u);
      wgpu::Buffer alive_buf = ph.upload(alive0.data(), alive0.size() * sizeof(uint32_t),
          wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopyDst);
      ...
      // PASS A: bin_count.
      ph.dispatch(pipe_bc, binds, wg ? wg : 1u, 1u, 1u);
      // prefix-sum on host.
      {
        wgpu::Buffer s = ph.alloc_staging_for_readback(cc_bytes);
        ph.copy_to_staging(cellcnt, s, cc_bytes);
        std::vector<uint8_t> b = ph.readback(s, cc_bytes);
      ...
      // PASS B: bin_scatter.
      ph.dispatch(pipe_bs, binds, wg ? wg : 1u, 1u, 1u);
      // PASS C: suppress, iterated to fixed point.
      const size_t keep_bytes = (size_t)n * sizeof(uint32_t);
      wgpu::Buffer keep_stage = ph.alloc_staging_for_readback(keep_bytes);
      std::vector<uint32_t> cur((size_t)n, 1u);
      for (int it = 0; it < 8; ++it) {
        ph.dispatch(pipe_su, binds, wg ? wg : 1u, 1u, 1u);
        ph.copy_to_staging(keep_buf, keep_stage, keep_bytes);
        std::vector<uint8_t> kb = ph.readback(keep_stage, keep_bytes);
    ...
    {
      auto pipe_aff = ph.load_compute(affine_wgsl, "affine_shape");
      wgpu::Buffer oct_buf = ph.upload(octgeom.data(), octgeom.size() * sizeof(OctGeom),
                                            wgpu::BufferUsage::Storage);
      wgpu::Buffer frame_buf = ph.upload(aff_in.data(), aff_in.size() * sizeof(InFrame),
                                              wgpu::BufferUsage::Storage);
      const size_t obytes = (size_t)Naff * sizeof(AffineShape);
      wgpu::Buffer out_buf = ph.alloc(obytes,
          wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc);
      ...
      ph.dispatch(pipe_aff, {pyramid, oct_buf, frame_buf, out_buf, P_buf},
                       Naff ? Naff : 1u, 1u, 1u);
      T.affine_ms = ms_since(ta);
      if (Naff) {
        wgpu::Buffer s = ph.alloc_staging_for_readback(obytes);
        ph.copy_to_staging(out_buf, s, obytes);
        std::vector<uint8_t> b = ph.readback(s, obytes);
    ...
    {
      auto pipe_or = ph.load_compute(orient_wgsl, "orient");
      wgpu::Buffer kp_buf = ph.upload(or_in.data(), or_in.size() * sizeof(InputKp),
                                           wgpu::BufferUsage::Storage);
      wgpu::Buffer geom_buf = ph.upload(octgeomO.data(), octgeomO.size() * sizeof(OctaveGeomO),
                                             wgpu::BufferUsage::Storage);
      uint32_t z = 0;
      wgpu::Buffer count_buf = ph.upload(&z, sizeof(z),
          wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc);
      ...
      ph.dispatch(pipe_or, {pyramid, kp_buf, geom_buf, count_buf, out_buf, P_buf},
                       gx ? gx : 1u, 1u, 1u);
      T.orient_ms = ms_since(to);
      uint32_t or_total = 0;
      {
        wgpu::Buffer s = ph.alloc_staging_for_readback(sizeof(uint32_t));
        ph.copy_to_staging(count_buf, s, sizeof(uint32_t));
        std::vector<uint8_t> b = ph.readback(s, sizeof(uint32_t));
      ...
      if (kept) {
        ...
        wgpu::Buffer s = ph.alloc_staging_for_readback(kb);
        ph.copy_to_staging(out_buf, s, kb);
        std::vector<uint8_t> b = ph.readback(s, kb);
    ...
    T.used_gpu_warp = false;  // this harness has no 2-kernel GPU-warp path
    {
      ...
      // ───────────────────────────────────────────────────────────────────────
      if (fused) {
        auto pipe_fused = ph.load_compute(fused_wgsl, "descriptor_fused");
        wgpu::Buffer lut_buf = ph.upload(lut.data(), lut.size() * sizeof(float),
                                              wgpu::BufferUsage::Storage);
        ...
        wgpu::Buffer octw_buf = ph.upload(octw.data(), octw.size() * sizeof(OctGeomW),
                                               wgpu::BufferUsage::Storage);
        ...
        wgpu::Buffer win_buf = ph.upload(wins.data(), n_recs * sizeof(WarpIn),
                                              wgpu::BufferUsage::Storage);
        const size_t out_words = (size_t)N * 32;
        wgpu::Buffer out_buf = ph.alloc(out_words * sizeof(uint32_t),
            wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc);
        const size_t dbg_n = (size_t)N * 128;
        wgpu::Buffer dbg_buf = ph.alloc(dbg_n * sizeof(float),
            wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc);
        FusedParams P{};
        ...
        ph.dispatch(pipe_fused,
                         {pyramid, win_buf, octw_buf, lut_buf, out_buf, P_buf, dbg_buf},
                         wg ? wg : 1u, 1u, 1u);
        desc_dispatch_ms += ms_since(tdd);
        wgpu::Buffer s = ph.alloc_staging_for_readback(out_words * sizeof(uint32_t));
        ph.copy_to_staging(out_buf, s, out_words * sizeof(uint32_t));
        std::vector<uint8_t> raw = ph.readback(s, out_words * sizeof(uint32_t));
      ...
      // ───────────────────────────────────────────────────────────────────────
      {
      auto pipe_desc = ph.load_compute(desc_wgsl, "descriptor");
      wgpu::Buffer lut_buf = ph.upload(lut.data(), lut.size() * sizeof(float),
                                            wgpu::BufferUsage::Storage);
      ...
      while (done < N) {
        ...
        wgpu::Buffer gss_buf = ph.upload(concat.data(), concat.size() * sizeof(float),
                                              wgpu::BufferUsage::Storage);
        wgpu::Buffer recs_buf = ph.upload(recs.data(), recs.size() * sizeof(PatchRec),
                                               wgpu::BufferUsage::Storage);
        ...
        auto tdd = clock_t_::now();
        ph.dispatch(pipe_desc, {gss_buf, recs_buf, lut_buf, out_buf, P_buf, dbg_buf},
                         wg ? wg : 1u, 1u, 1u);
        desc_dispatch_ms += ms_since(tdd);
        wgpu::Buffer s = ph.alloc_staging_for_readback(out_words * sizeof(uint32_t));
        ph.copy_to_staging(out_buf, s, out_words * sizeof(uint32_t));
    ...
    T.prof = g_prof;              // S1: capture full-frame profiling accumulators
    T.gss_readback_done = false;  // INVARIANT: resident gss never read back.

    // ── S1 per-frame profiling print (wall-clock attribution) ──
    {
      const Prof& P = T.prof;
      const double gpu_attributed =
          P.dispatch_submit_wait + P.copy_submit_wait + P.readback_map_wait +
          P.upload_ms + P.alloc_ms + P.pipeline_ms + P.raw_submit_ms;
      const double host_glue = T.total_ms - gpu_attributed;
      // post-gss split (whole-frame minus gss-build deltas).
      const double post_dispatch = P.dispatch_submit_wait - T.gss_dispatch_ms;
      const double post_copy = P.copy_submit_wait - T.gss_copy_ms;
      const double post_readback = P.readback_map_wait - T.gss_readback_ms;
      const double post_upload = P.upload_ms - T.gss_upload_ms;
      const double post_alloc = P.alloc_ms - T.gss_alloc_ms;
      const double post_pipeline = P.pipeline_ms - T.gss_pipeline_ms;
      auto pct = [&](double v) { return T.total_ms > 0 ? 100.0 * v / T.total_ms : 0.0; };
      std::printf("\n  ===== S1 WALL-CLOCK ATTRIBUTION (this frame) =====\n");
      std::printf("    FRAME TOTAL (total_ms)        : %8.1f ms  (100%%)\n", T.total_ms);
      // NOTE: the 2 gss pipeline compiles (blur+resample) happen BEFORE the B.1
      // wall-clock t0 starts, so gss_pipeline_ms is NOT inside gss_build_ms (it
      // lives in the top-level (e) Tint bucket). gss_build_ms ~= gss_dispatch
      // (the blur kernels' submit+wait) + the fire-and-forget CopyBuf submits +
      // the final flush. We report the deltas for transparency.
      std::printf("    -- gss build (B.1) bucket     : %8.1f ms  (%.1f%%)\n", T.gss_build_ms, pct(T.gss_build_ms));
      std::printf("         gss dispatch submit+wait : %8.1f ms   (blur kernels — the bulk of the bucket)\n", T.gss_dispatch_ms);
      std::printf("         gss flush copy submit+wt : %8.1f ms\n", T.gss_copy_ms);
      std::printf("         gss flush map readback   : %8.1f ms\n", T.gss_readback_ms);
      std::printf("         gss upload (taps/params) : %8.1f ms\n", T.gss_upload_ms);
      std::printf("         gss alloc                : %8.1f ms\n", T.gss_alloc_ms);
      std::printf("         gss raw-copy + remainder : %8.1f ms   (CopyBuf submits + host setup, inside bucket)\n",
                  T.gss_build_ms - (T.gss_dispatch_ms + T.gss_copy_ms + T.gss_readback_ms + T.gss_upload_ms + T.gss_alloc_ms));
      std::printf("         [gss pipeline compile    : %8.1f ms  -> counted in top-level (e) Tint, NOT this bucket]\n", T.gss_pipeline_ms);
      std::printf("    -- POST-gss pipeline categories --\n");
      std::printf("    (a) dispatch Submit+wait      : %8.1f ms  (%.1f%%)  [%d dispatches: suppress/affine/orient/desc]\n",
                  post_dispatch, pct(post_dispatch),
                  P.n_dispatch /*total*/);
      std::printf("    (a') raw-submit (detect/refine): %8.1f ms  (%.1f%%)  [%d raw submits, fire-and-forget]\n",
                  P.raw_submit_ms, pct(P.raw_submit_ms), P.n_raw_submit);
      std::printf("    (b) copy_to_staging Submit+wait: %8.1f ms  (%.1f%%)  [%d copies]\n",
                  post_copy, pct(post_copy), P.n_copy /*total*/);
      std::printf("    (c) readback MapAsync+memcpy   : %8.1f ms  (%.1f%%)  [%d readbacks]\n",
                  post_readback, pct(post_readback), P.n_readback /*total*/);
      std::printf("    (d) upload (CreateBuf+Write)   : %8.1f ms  (%.1f%%)  [%d uploads]\n",
                  post_upload, pct(post_upload), P.n_upload /*total*/);
      std::printf("    (d') alloc (CreateBuffer)      : %8.1f ms  (%.1f%%)  [%d allocs]\n",
                  post_alloc, pct(post_alloc), P.n_alloc /*total*/);
      std::printf("    (e) pipeline compile (Tint)    : %8.1f ms  (%.1f%%)  [%d compiles]\n",
                  post_pipeline, pct(post_pipeline), P.n_pipeline /*total*/);
      std::printf("    (f) HOST CPU GLUE (remainder)  : %8.1f ms  (%.1f%%)  [sort/compaction/OFrame/WarpIn/parity-prep]\n",
                  host_glue, pct(host_glue));
      std::printf("    ----------------------------------------------------\n");
      std::printf("    SUM of GPU-attributed buckets  : %8.1f ms  (%.1f%%)\n",
                  gpu_attributed, pct(gpu_attributed));
      std::printf("    TOTAL Submit+wait points/frame : %d  (= %d dispatch + %d copy_to_staging + %d raw-submit)\n",
                  P.n_dispatch + P.n_copy + P.n_raw_submit, P.n_dispatch, P.n_copy, P.n_raw_submit);
      std::printf("    TOTAL CPU MapAsync readbacks   : %d\n", P.n_readback);
      // ── projected wall-clock under the two re-architecture moves ──
      const double residual_no_compile = T.total_ms - P.pipeline_ms;  // pipelines cached at init
      const double pure_gpu = P.dispatch_submit_wait + P.raw_submit_ms;  // kernels only
      const double readback_overhead = P.copy_submit_wait + P.readback_map_wait;
      std::printf("    --------------------------------------------------\n");
      std::printf("    PROJECTION (1) pipelines pre-compiled at init (remove (e) Tint):\n");
      std::printf("        residual frame wall-clock  : %8.1f ms   (= total - Tint compile)\n", residual_no_compile);
      std::printf("    PROJECTION (2) + indirect dispatch removes count readbacks (remove (b)+(c)):\n");
      std::printf("        residual frame wall-clock  : %8.1f ms   (= above - copy(%.1f) - readback(%.1f))\n",
                  residual_no_compile - readback_overhead, P.copy_submit_wait, P.readback_map_wait);
      std::printf("        of which PURE GPU kernels  : %8.1f ms   (dispatch+raw submit-wait; the irreducible floor)\n", pure_gpu);
      std::printf("  ==================================================\n");
    }
