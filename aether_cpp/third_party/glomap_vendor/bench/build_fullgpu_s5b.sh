#!/usr/bin/env bash
# build_fullgpu_s5b.sh — standalone build of the S5b RE-INTEGRATION harness
# (bench/extract_fullgpu_s5b.cc): the full-GPU DSP-SIFT pipeline with the
# descriptor warp-setup moved ON-GPU (sift_descriptor_warp.wgsl). Mirrors
# build_fullgpu.sh exactly; unique obj dir /tmp/fullgpu_s5b_obj so it never
# collides with the per-stage / fullgpu harnesses. Run from aether_cpp/ root.
set -euo pipefail
...
# S5b integration harness TU.
c++ -std=c++20 -arch arm64 -O2 \
  -I "$ROOT/tools" -I "$DAWN_INC_SRC" -I "$DAWN_INC_GEN" \
  -I "$VLF" -I "$ROOT/third_party/stb" \
  -c "$ROOT/third_party/glomap_vendor/bench/extract_fullgpu_s5b.cc" \
  -o "$OBJ/extract_fullgpu_s5b.o"
...
c++ -std=c++20 -arch arm64 -g -Wl,-search_paths_first -Wl,-headerpad_max_install_names \
  "$OBJ/extract_fullgpu_s5b.o" "$OBJ/dawn_kernel_harness.o" \
  ...
  -o "$OBJ/extract_fullgpu_s5b_exe"

echo "built: $OBJ/extract_fullgpu_s5b_exe"
echo "run (from $ROOT):"
echo "  # host fp64 warp-setup (placeholder baseline):"
echo "  $OBJ/extract_fullgpu_s5b_exe \\"
...
echo "  # S5b on-GPU warp-setup:"
echo "  $OBJ/extract_fullgpu_s5b_exe --gpu-warp \\"
echo "    third_party/glomap_vendor/iosapp/Resources/sift_test.jpg \\"
echo "    third_party/glomap_vendor/iosapp/Resources/sift_test2.jpg"
