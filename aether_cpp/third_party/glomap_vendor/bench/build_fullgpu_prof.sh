#!/usr/bin/env bash
# build_fullgpu_prof.sh — PROFILING copy of the S5c fusion harness
# (bench/extract_fullgpu_prof.cc): adds fine CPU timers to attribute the
# per-frame wall-clock. Mirrors build_fullgpu_fused.sh; unique obj dir
# /tmp/fullgpu_prof_obj. Run from aether_cpp/ root.
set -euo pipefail
...
c++ -std=c++20 -arch arm64 -O2 \
  ...
  -c "$ROOT/third_party/glomap_vendor/bench/extract_fullgpu_prof.cc" \
  -o "$OBJ/extract_fullgpu_prof.o"

c++ -std=c++20 -arch arm64 -g -Wl,-search_paths_first -Wl,-headerpad_max_install_names \
  "$OBJ/extract_fullgpu_prof.o" "$OBJ/dawn_kernel_harness.o" \
  ...
  -o "$OBJ/extract_fullgpu_prof_exe"

echo "built: $OBJ/extract_fullgpu_prof_exe"
