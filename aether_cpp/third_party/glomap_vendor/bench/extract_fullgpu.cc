...
//
// extract_fullgpu.cc — FULL-GPU DSP-SIFT extraction integration harness (the
// climax of the GPU DSP-SIFT port). Chains ALL validated GPU stages into ONE
// pipeline with the Gaussian scale-space (gss) kept RESIDENT on the GPU the
// whole frame (built once, re-read by detect/refine/affine/orient — NEVER
// rebuilt or round-tripped), ending in ONE readback of the final keypoints +
// 128-D descriptors. Measures e2e @4224 vs the production-exact CPU baseline and
// validates the end-to-end features against VLFeat's full vl_covdet extraction.
//
// PIPELINE (per GPU_DSP_SIFT_PLAN.md data flow):
//   gss(RESIDENT) -> DoG+extremum -> refine+gate -> suppression
//                 -> affine-shape -> orientation(1-4x expand) -> descriptor
//
// ── gss RESIDENCY (the load-bearing constraint) ──
//   The pyramid is assembled into ONE big GPU buffer (all octaves, all
//   subdivisions [firstSub..lastSub], level-major, per-octave base offset). The
//   blur/resample kernels write level scratch buffers; CopyBufferToBuffer moves
//   each finished level into the resident pyramid slice — NO per-level CPU
//   readback (unlike the S1 gss-build harness, whose gpu_blur read every level
//   back, inflating gss_ms; we report the REAL resident-gss build time here).
//   detect/refine bind a SUB-SLICE of the resident buffer per octave via a
//   per-octave bind group with a buffer OFFSET; affine/orient bind the WHOLE
//   resident buffer + the octave-geometry table. The resident buffer is read by
//   4 stages and never leaves the GPU.
//
// ── THE DESCRIPTOR BOUNDARY (honest blocker; see report) ──
//   The validated descriptor kernel (sift_dsp_descriptor.wgsl) does NOT read the
//   resident gss directly: its per-(kp x scale) AFFINE-SVD + fp64 scale-space
//   LEVEL PICK + variable-size patch PADDING (covdet.c:2222-2360) are done
//   HOST-SIDE (parity_descriptor.cc build_warp_setup; the shader header marks
//   this deliberate — the on-GPU warp-setup is S5b, unbuilt). So the chain runs
//   FULLY GPU and gss-RESIDENT through ORIENTATION; the descriptor then consumes
//   the GPU oriented+affine keypoints via the SAME validated host warp-setup
//   bridge + per-rec padded planes (NOT the resident gss). This is reported as
//   the exact integration blocker. The resident gss has NO readback regardless
//   (the warp-setup reads VLFeat's CPU gss for the bridge, a SEPARATE buffer; the
//   GPU resident pyramid is confirmed never read back).
//
// Build: bench/build_fullgpu.sh (worktree-only; unique obj dir /tmp/fullgpu_obj;
// reuses the prebuilt host Dawn + the VLFeat .c subset + tools/
// dawn_kernel_harness.cpp read-only). Run from aether_cpp/ root:
//   /tmp/fullgpu_obj/extract_fullgpu_exe \
...
struct GpuTimings {
  ...
  double warp_setup_ms = 0;     // host fp64 warp setup (descriptor bridge)
...
int main(int argc, char** argv) {
  ...
  for (const std::string& imgpath : imgs) {
    ...
    // ════════════════════════════════════════════════════════════════════════
    //  (B.7) DESCRIPTOR — host fp64 warp-setup bridge + GPU resample/descriptor.
    //  *** This is the integration BOUNDARY: the validated descriptor cannot
    //      read the resident gss; its warp setup (fp64 level-pick + padding) is
    //      host-side. We use the SAME validated bridge (build_warp_setup) over
    //      VLFeat's CPU gss, then the GPU descriptor kernel. The resident GPU
    //      pyramid is NEVER read back (the bridge reads the separate CPU gss). ***
    //  2GB Dawn limit: the concatenated padded planes (N*10 recs) can exceed
    //  2 GB; we BATCH dispatches when the concat would cross the limit
    //  (~11k kp/dispatch, per parity_descriptor.cc).
    // ════════════════════════════════════════════════════════════════════════
    std::vector<uint8_t> gpu_desc((size_t)RES.final_kp * 128);
    {
      ...
      wgpu::Buffer lut_buf = harness.upload(lut.data(), lut.size() * sizeof(float),
                                            wgpu::BufferUsage::Storage);
      const double dsp_step = (kDspMaxScale - kDspMinScale) / kDspNumScales;
      VlScaleSpace* gss_cpu = vl_covdet_get_gss(cd);  // SEPARATE CPU gss (bridge)

      const int N = (int)RES.final_kp;
      // 2GB batching: cap floats per dispatch at ~480M (~1.92 GB) to stay under
      // the 2 GB binding limit with headroom.
      const size_t kMaxConcatFloats = 480ull * 1000 * 1000;
      int done = 0;
      double warp_ms = 0, desc_dispatch_ms = 0;
      int n_batches = 0;
      while (done < N) {
        // grow a batch until the concat would exceed the float cap.
        std::vector<PatchRec> recs;
        std::vector<float> concat;
        int b0 = done;
        auto tw = clock_t_::now();
        while (done < N) {
          const OFrame& f = finals[done];
          // build the 10 dsp-scale warps for this kp; measure their concat size.
          ...
          (void)ok;
          // would this kp overflow the batch cap? (always allow at least 1 kp)
          if (!recs.empty() && concat.size() + add > kMaxConcatFloats) break;
          // commit the kp's recs (fix level_off relative to this batch's concat).
          for (int s = 0; s < kDspNumScales; ++s) {
        ...
        // GPU descriptor dispatch for this batch.
        ...
        // readback this batch's descriptors.
      ...
      std::printf("[B.7 descriptor] host warp-setup=%.1f ms  GPU desc dispatch=%.1f ms  (%d kp, %d batch(es))\n",
  ...
  std::printf("    descriptor: host warp  : %8.1f ms   *** BOUNDARY: fp64 warp setup on CPU (S5b unbuilt) ***\n", s_warp);
  ...
  // descriptor-warp-on-GPU headroom (S5b): subtract the host warp setup (the
  // only non-GPU stage) to show the ceiling if the warp setup moves to the GPU.
