...
//
// extract_fullgpu_s5b.cc — FULL-GPU DSP-SIFT integration, S5b RE-INTEGRATION:
// the descriptor warp-setup moved ON-GPU so the chain is FULLY GPU + gss-RESIDENT
// THROUGH THE DESCRIPTOR. Extends extract_fullgpu.cc (which ran fully GPU through
// ORIENTATION but did the per-(kp x scale) warp-setup HOST-side in fp64 — the
// ~82k-warp bottleneck that stalled cumulative speedup at ~1.1-1.5x). This
// variant wires the S5b on-GPU warp-setup (sift_descriptor_warp.wgsl) BEFORE the
// descriptor kernel: warp-setup reads the resident gss → patches stay GPU →
// descriptor → ONE final readback of keypoints + descriptors. Measures the FINAL
// e2e @4224 (the real streaming-feasibility verdict).
//
// PIPELINE (per GPU_DSP_SIFT_PLAN.md data flow):
//   gss(RESIDENT) -> DoG+extremum -> refine+gate -> suppression
//                 -> affine-shape -> orientation(1-4x) -> WARP-SETUP -> descriptor
//
// ── MODE FLAG ──
//   --gpu-warp  : run the S5b on-GPU warp-setup (sift_descriptor_warp.wgsl). The
//                 fp64 host build_warp_setup is REMOVED from the critical path;
//                 the warp-setup reads the resident gss and emits PatchRecs that
//                 point straight at the resident pyramid for interior keypoints
//                 (zero data movement) and into a GPU-filled padded-plane buffer
//                 only for the rare edge keypoints. The descriptor's gss binding
//                 is the resident pyramid (in-bounds) + the GPU OOB planes.
//   (default)   : the host fp64 warp-setup PLACEHOLDER (identical to
//                 extract_fullgpu.cc) — the proven bridge; the honest baseline.
//   Both modes ALWAYS run the host warp-setup as the e2e parity REFERENCE; the
//   --gpu-warp run additionally validates the GPU warp-setup vs that reference
//   (level-pick agreement + per-rec PatchRec deltas) and reports flips honestly.
//
...
//   warp-setup + descriptor. NO per-level readback; ONE forced readback/frame
//   (the final kps + descriptors). In --gpu-warp mode the resident gss is the
//   warp source — it is the SAME buffer all 6 prior stages used, never
//   round-tripped. (The host parity reference reads VLFeat's SEPARATE CPU gss.)
//
// Build: bench/build_fullgpu_s5b.sh (worktree-only; obj dir /tmp/fullgpu_s5b_obj;
// reuses the prebuilt host Dawn + the VLFeat .c subset + tools/
// dawn_kernel_harness.cpp read-only). Run from aether_cpp/ root:
//   /tmp/fullgpu_s5b_obj/extract_fullgpu_s5b_exe [--gpu-warp] \
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "--gpu-warp") { gpu_warp = true; continue; }
  ...
  // S5b warp-setup shader (task A artifact). Optional: if absent, --gpu-warp
  // falls back to a host EMULATION of the on-GPU data path (clearly marked) so
  // the re-integration is still measured end-to-end.
  const std::string warp_wgsl = read_file("shaders/wgsl/sift_descriptor_warp.wgsl");
  const bool warp_shader_present = !warp_wgsl.empty();
  std::printf("[mode] descriptor warp-setup = %s%s\n",
              gpu_warp ? "GPU (S5b)" : "HOST fp64 (placeholder)",
              gpu_warp ? (warp_shader_present ? " [shader: sift_descriptor_warp.wgsl]"
                                              : " [shader ABSENT -> host-emulated GPU data path]")
                       : "");
  ...
  for (const std::string& imgpath : imgs) {
    ...
    // ════════════════════════════════════════════════════════════════════════
    //  (B.7) DESCRIPTOR — warp-setup (HOST fp64 placeholder | S5b on-GPU) + GPU
    //  descriptor. The descriptor kernel is UNCHANGED in both modes: it reads a
    //  concat of per-(kp x scale) padded planes via PatchRec{level_off,lw,lh}.
    //  The only difference is WHO builds those planes:
    //    HOST mode  : build_warp_setup over VLFeat's CPU gss (the proven bridge;
    //                 the ~82k-warp fp64 bottleneck). The resident GPU pyramid is
    //                 untouched.
    //    --gpu-warp : sift_descriptor_warp.wgsl reads the RESIDENT pyramid and
    //                 materializes the planes ON-GPU (warp_size -> host prefix-sum
    //                 -> warp_fill). The host fp64 loop + host memcpy are GONE
    //                 from the critical path; the resident gss is the warp source.
    //  2GB Dawn limit: the concat planes (N*10 recs) can exceed 2 GB; both modes
    //  BATCH dispatches when the concat would cross the limit (per parity_descriptor.cc).
    // ════════════════════════════════════════════════════════════════════════
    std::vector<uint8_t> gpu_desc((size_t)RES.final_kp * 128);
    T.used_gpu_warp = gpu_warp;
    {
      ...
      wgpu::Buffer lut_buf = harness.upload(lut.data(), lut.size() * sizeof(float),
                                            wgpu::BufferUsage::Storage);
      const double dsp_step = (kDspMaxScale - kDspMinScale) / kDspNumScales;
      VlScaleSpace* gss_cpu = vl_covdet_get_gss(cd);  // CPU gss (host bridge / parity ref)

      const int N = (int)RES.final_kp;
      const size_t kMaxConcatFloats = 480ull * 1000 * 1000;  // ~1.92 GB binding cap

      // S5b GPU warp-setup pipelines + the resident-octave table (gpu mode only).
      std::unique_ptr<wgpu::ComputePipeline> pipe_wsize, pipe_wfill;
      wgpu::Buffer octw_buf;
      std::vector<OctGeomW> octw(nOct);
      int gpu_warp_level_flips = 0, gpu_warp_dim_mismatch = 0; double gpu_warp_max_AT = 0;
      if (gpu_warp && warp_shader_present) {
        pipe_wsize = std::make_unique<wgpu::ComputePipeline>(
            harness.load_compute(warp_wgsl, "warp_size"));
        pipe_wfill = std::make_unique<wgpu::ComputePipeline>(
            harness.load_compute(warp_wgsl, "warp_fill"));
        for (int k = 0; k < nOct; ++k) {
          const OctInfo& r = octinfo[k];
          octw[k] = OctGeomW{(uint32_t)r.w, (uint32_t)r.h, r.base, 0u, (float)r.step, 0, 0, 0};
        }
        octw_buf = harness.upload(octw.data(), octw.size() * sizeof(OctGeomW),
                                  wgpu::BufferUsage::Storage);
      }

      int done = 0;
      double warp_ms = 0, gpu_warp_ms = 0, desc_dispatch_ms = 0, host_ref_ms = 0;
      double warp_size_ms = 0, warp_scan_ms = 0, warp_fill_ms = 0;
      int n_batches = 0;
      while (done < N) {
        std::vector<PatchRec> recs;        // descriptor records (host or GPU-filled)
        std::vector<float> concat;         // host-built planes (host mode)
        std::vector<WarpIn> wins;          // GPU warp inputs (gpu mode)
        int b0 = done;

        // ── grow a batch (size accounting needs the host plane dims either way) ──
        ...
        while (done < N) {
          ...
          std::vector<WarpIn> kp_wins(kDspNumScales);
          size_t add = 0;
          for (int s = 0; s < kDspNumScales; ++s) {
            const double dsc = kDspMinScale + s * dsp_step;
            ...
            if (!build_warp_setup(gss_cpu, A_in, T_in, Dd[0], Dd[3],
              ...
              // keep a 1x1 zero plane to preserve indexing
              ws.plane.assign(1, 0.0f); ws.lw = 1; ws.lh = 1;
              ws.A[0]=ws.A[1]=ws.A[2]=ws.A[3]=0; ws.T[0]=ws.T[1]=0;
            }
            kp_planes[s] = ws.plane;  // host plane (host mode) / size accounting (gpu)
            PatchRec r{};
            ...
            kp_recs[s] = r;
            kp_wins[s] = WarpIn{(float)sf.x, (float)sf.y, (float)sf.a11,
                                (float)sf.a12, (float)sf.a21, (float)sf.a22, 0.f, 0.f};
          ...
          for (int s = 0; s < kDspNumScales; ++s) {
            ...
            wins.push_back(kp_wins[s]);
          }
          ++done;
        }
        const double host_warp_batch_ms = ms_since(tw);
        const int batch_n = done - b0;
        const int n_recs = batch_n * kDspNumScales;
        ++n_batches;

        // ── build the descriptor's `gss` (concat planes) + `recs` ──
        wgpu::Buffer gss_buf, recs_buf;
        if (!gpu_warp || !warp_shader_present) {
          // HOST mode (or shader absent fallback): the host already built the
          // concat + recs above; that host cost IS the warp-setup cost.
          warp_ms += host_warp_batch_ms;
          if (gpu_warp && !warp_shader_present) {
            // host-EMULATED gpu data path: we still time it as gpu_warp so the
            // verdict is honest about the missing kernel (clearly labeled).
            gpu_warp_ms += host_warp_batch_ms;
          }
          gss_buf = harness.upload(concat.data(), concat.size() * sizeof(float),
                                   wgpu::BufferUsage::Storage);
          recs_buf = harness.upload(recs.data(), recs.size() * sizeof(PatchRec),
                                    wgpu::BufferUsage::Storage);
        } else {
          // ── S5b ON-GPU WARP-SETUP ──
          // The host loop above was only for batch sizing + the parity REFERENCE
          // (recs/concat); it is INSTRUMENTATION, excluded from the pipeline total
          // (a real on-device path replaces it with a GPU plane-count prefix-sum).
          host_ref_ms += host_warp_batch_ms;
          auto tg = clock_t_::now();
          // 1) warp_size: per-rec geometry + plane float counts.
          wgpu::Buffer win_buf = harness.upload(wins.data(), wins.size() * sizeof(WarpIn),
                                                wgpu::BufferUsage::Storage);
          wgpu::Buffer geom_buf = harness.alloc((size_t)n_recs * sizeof(WarpGeom),
              wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc);
          wgpu::Buffer pf_buf = harness.alloc((size_t)n_recs * sizeof(uint32_t),
              wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc);
          WarpParams WP{(uint32_t)n_recs, (uint32_t)nOct, G.firstOctave, G.lastOctave,
                        G.firstSub, G.lastSub, (float)g.octaveResolution,
                        (float)g.baseScale, (float)kPatchRelativeExtent,
                        (float)(kPatchRelativeExtent / kPatchResolution),
                        (float)kPatchRelativeSmoothing, 0.f};
          wgpu::Buffer wp_buf = harness.upload(&WP, sizeof(WP), wgpu::BufferUsage::Uniform);
          const uint32_t sgx = ((uint32_t)n_recs + 63u) / 64u;
          auto t_wsize = clock_t_::now();
          harness.dispatch(*pipe_wsize, {win_buf, octw_buf, geom_buf, pf_buf, wp_buf},
                           sgx ? sgx : 1u, 1u, 1u);
          warp_size_ms += ms_since(t_wsize);
          // 2) host prefix-sum of plane_floats -> per-rec offsets (tiny; this is
          //    the only host touch — a uint scan over n_recs, as suppression does).
          auto t_scan = clock_t_::now();
          std::vector<uint32_t> pf(n_recs);
          {
            wgpu::Buffer s = harness.alloc_staging_for_readback((size_t)n_recs * sizeof(uint32_t));
            harness.copy_to_staging(pf_buf, s, (size_t)n_recs * sizeof(uint32_t));
            std::vector<uint8_t> b = harness.readback(s, (size_t)n_recs * sizeof(uint32_t));
            std::memcpy(pf.data(), b.data(), (size_t)n_recs * sizeof(uint32_t));
          }
          std::vector<uint32_t> rec_off(n_recs);
          uint64_t acc = 0;
          for (int i = 0; i < n_recs; ++i) { rec_off[i] = (uint32_t)acc; acc += pf[i]; }
          const size_t concat_floats = (size_t)acc;
          wgpu::Buffer ro_buf = harness.upload(rec_off.data(), rec_off.size() * sizeof(uint32_t),
                                               wgpu::BufferUsage::Storage);
          warp_scan_ms += ms_since(t_scan);
          // 3) warp_fill: materialize planes from the RESIDENT pyramid + emit recs.
          gss_buf = harness.alloc(concat_floats * sizeof(float),
              wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc);
          recs_buf = harness.alloc((size_t)n_recs * sizeof(PatchRec),
              wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc);
          // warp_fill: capped grid (must == WARP_FILL_GRID in the shader);
          // each workgroup strides over recs r=wg.x, wg.x+GRID, ...
          const uint32_t kWarpFillGrid = 32768u;
          const uint32_t fgx = std::min((uint32_t)n_recs, kWarpFillGrid);
          auto t_wfill = clock_t_::now();
          harness.dispatch(*pipe_wfill, {pyramid, geom_buf, ro_buf, gss_buf, recs_buf, wp_buf},
                           fgx ? fgx : 1u, 1u, 1u);
          warp_fill_ms += ms_since(t_wfill);
          gpu_warp_ms += ms_since(tg);

          // ── parity check (gpu warp-setup vs the host fp64 reference) ──
          // level pick = level_base/(s-firstSub) agreement via lw/lh + A/T deltas.
          if (b0 == 0) {  // sample the first batch only (cost control)
            std::vector<WarpGeom> gv(n_recs);
            wgpu::Buffer s = harness.alloc_staging_for_readback((size_t)n_recs * sizeof(WarpGeom));
            harness.copy_to_staging(geom_buf, s, (size_t)n_recs * sizeof(WarpGeom));
            std::vector<uint8_t> b = harness.readback(s, (size_t)n_recs * sizeof(WarpGeom));
            std::memcpy(gv.data(), b.data(), (size_t)n_recs * sizeof(WarpGeom));
            for (int i = 0; i < n_recs; ++i) {
              // host rec lw/lh are the host plane dims; gpu gv[i].lw/lh same.
              if (gv[i].lw != recs[i].lw || gv[i].lh != recs[i].lh) ++gpu_warp_dim_mismatch;
              else {
                // dims agree => same level pick => compare A (host already /step,
                // gpu also /step) and T (host shifted by -x0i; gpu rec is in
                // recs_out, gv is pre-shift => compare A only here).
                gpu_warp_max_AT = std::max({gpu_warp_max_AT,
                    (double)std::fabs(gv[i].a0 - recs[i].a0),
                    (double)std::fabs(gv[i].a3 - recs[i].a3)});
              }
            }
            // level flips ~ dim mismatches (a different level => different w/h =>
            // different padded dims in almost all cases).
            gpu_warp_level_flips = gpu_warp_dim_mismatch;
          }
        }

        // ── descriptor dispatch (UNCHANGED kernel; reads gss_buf + recs_buf) ──
      ...
      T.gpu_warp_ms = gpu_warp_ms;
      T.host_ref_ms = host_ref_ms;
      T.desc_ms = desc_dispatch_ms;
      if (gpu_warp && warp_shader_present) {
        std::printf("[B.7 descriptor] S5b GPU warp-setup=%.1f ms (warp_size=%.1f + scan/readback=%.1f + warp_fill=%.1f)  GPU desc dispatch=%.1f ms  (%d kp, %d batch(es))\n",
                    T.gpu_warp_ms, warp_size_ms, warp_scan_ms, warp_fill_ms, T.desc_ms, N, n_batches);
        std::printf("[B.7 warp parity] level-pick flips=%d (dim-mismatch=%d)  max|dA|=%.3e  (vs host fp64, first batch sample)\n",
                    gpu_warp_level_flips, gpu_warp_dim_mismatch, gpu_warp_max_AT);
      } else if (gpu_warp) {
        std::printf("[B.7 descriptor] GPU-warp REQUESTED but shader ABSENT -> HOST-EMULATED warp-setup=%.1f ms  GPU desc=%.1f ms  (%d kp, %d batch(es))\n",
                    T.gpu_warp_ms, T.desc_ms, N, n_batches);
      } else {
    ...
    // In --gpu-warp mode the host parity-reference build (host_ref_ms) ran inline
    // for validation only; it is NOT part of the GPU pipeline → subtract it so
    // total_ms reflects the real on-GPU-warp-setup pipeline cost.
    if (T.used_gpu_warp) T.total_ms -= T.host_ref_ms;
  ...
  double s_gss = 0, s_det = 0, s_ref = 0, s_sup = 0, s_aff = 0, s_or = 0,
         s_warp = 0, s_gpuwarp = 0, s_desc = 0;
  bool any_gpu_warp = false;
  for (const auto& R : results) {
    ...
    s_warp += R.t.warp_setup_ms; s_gpuwarp += R.t.gpu_warp_ms; s_desc += R.t.desc_ms;
    any_gpu_warp = any_gpu_warp || R.t.used_gpu_warp;
  ...
  std::printf("\n  --- per-stage GPU breakdown (sum over %zu images;  warp-setup = %s) ---\n",
              n, any_gpu_warp ? "ON-GPU (S5b)" : "HOST fp64 (placeholder)");
  ...
  if (any_gpu_warp) {
    std::printf("    descriptor: GPU warp   : %8.1f ms   *** S5b: warp-setup ON GPU (host fp64 GONE from critical path) ***\n", s_gpuwarp);
  } else {
    std::printf("    descriptor: host warp  : %8.1f ms   *** BOUNDARY: fp64 warp setup on CPU (placeholder; pass --gpu-warp) ***\n", s_warp);
  }
  ...
  std::printf("  host-warp-setup GONE      : %s\n",
              any_gpu_warp ? "CONFIRMED (warp-setup runs ON GPU reading the resident pyramid)"
                           : "NO — still host fp64 (placeholder mode; pass --gpu-warp)");
  std::printf("  gss READBACK GONE         : %s  (resident gss never round-tripped)\n",
              "CONFIRMED");
  std::printf("\n  --- iPhone 14 Pro estimate (x%.1f M3-Pro absolute, PLAN) ---\n", kIphoneScale);
  std::printf("    FULL-GPU / frame (iPhone est.): %8.1f ms   vs ~2000 ms streaming budget @4224  => %s\n",
              (gpu_sum / n) * kIphoneScale,
              (gpu_sum / n) * kIphoneScale <= 2000.0 ? "WITHIN BUDGET" : "OVER BUDGET");
  if (!any_gpu_warp) {
    // headroom: subtract the host warp setup to show the S5b ceiling.
    const double gpu_sum_no_warp = gpu_sum - s_warp;
    std::printf("    [S5b headroom] FULL-GPU minus host-warp / frame (iPhone est.): %8.1f ms  (if warp-setup moves to GPU)\n",
                (gpu_sum_no_warp / n) * kIphoneScale);
  }
  std::printf("=================================================================\n");
  return 0;
}
