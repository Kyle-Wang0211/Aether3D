#!/usr/bin/env bash
# build_fullgpu.sh — standalone build of the FULL-GPU DSP-SIFT integration
# harness (bench/extract_fullgpu.cc), the climax of the GPU DSP-SIFT port:
# chains ALL validated GPU stages into one resident-gss pipeline.
#
# Mirrors build_descriptor.sh / build_gpuparity.sh: links the PREBUILT host Dawn
# archive (no Dawn rebuild) + the VLFeat .c subset the WHOLE pipeline needs
# (detector + scalespace + patch + gradient + raw SIFT descriptor + svd2 + math)
# + the read-only tools/dawn_kernel_harness.cpp. Unique obj dir /tmp/fullgpu_obj
# so it never collides with the per-stage harnesses. Run from aether_cpp/ root.
set -euo pipefail
...
OBJ="${OBJ:-/tmp/fullgpu_obj}"
mkdir -p "$OBJ"

VLDEFS="-DVL_DISABLE_SSE2 -DVL_DISABLE_AVX -DVL_DISABLE_OPENMP"

# VLFeat (arm64 scalar) — full pipeline units (detector + patch + gradient +
# raw descriptor + svd2/math), same set as build_descriptor.sh.
...
# Integration harness TU.
c++ -std=c++20 -arch arm64 -O2 \
  -I "$ROOT/tools" -I "$DAWN_INC_SRC" -I "$DAWN_INC_GEN" \
  -I "$VLF" -I "$ROOT/third_party/stb" \
  -c "$ROOT/third_party/glomap_vendor/bench/extract_fullgpu.cc" \
  -o "$OBJ/extract_fullgpu.o"
...
c++ -std=c++20 -arch arm64 -g -Wl,-search_paths_first -Wl,-headerpad_max_install_names \
  "$OBJ/extract_fullgpu.o" "$OBJ/dawn_kernel_harness.o" \
  ...
  -o "$OBJ/extract_fullgpu_exe"

echo "built: $OBJ/extract_fullgpu_exe"
echo "run (from $ROOT):"
echo "  $OBJ/extract_fullgpu_exe \\"
