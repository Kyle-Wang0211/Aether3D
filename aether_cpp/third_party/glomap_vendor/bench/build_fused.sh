#!/usr/bin/env bash
# build_fused.sh — standalone build of the GPU DSP-SIFT FUSED warp+descriptor
# parity harness (bench/parity_fused.cc), WITHOUT a full project CMake configure.
#
# Mirrors build_warp.sh (S5b): links the PREBUILT host Dawn archive (no Dawn
# rebuild) + the VLFeat .c subset the descriptor + warp-setup path needs. Run
# from aether_cpp/ root. Unique obj dir /tmp/parity_fused_obj so it never
# collides with the S1/S3/S4/S5b harnesses.
set -euo pipefail
...
c++ -std=c++20 -arch arm64 -O2 \
  ...
  -c "$ROOT/third_party/glomap_vendor/bench/parity_fused.cc" \
  -o "$OBJ/parity_fused.o"

# Link: monolithic libwebgpu_dawn.a + system frameworks (from the S5b harness).
c++ -std=c++20 -arch arm64 -g -Wl,-search_paths_first -Wl,-headerpad_max_install_names \
  "$OBJ/parity_fused.o" "$OBJ/dawn_kernel_harness.o" \
  ...
  -o "$OBJ/parity_fused_exe"

echo "built: $OBJ/parity_fused_exe"
echo "run (from $ROOT):"
echo "  $OBJ/parity_fused_exe \\"
echo "    third_party/glomap_vendor/iosapp/Resources/sift_test.jpg \\"
echo "    shaders/wgsl/sift_dsp_descriptor.wgsl \\"
echo "    shaders/wgsl/sift_descriptor_warp.wgsl \\"
echo "    shaders/wgsl/sift_descriptor_fused.wgsl [kp_cap]"
