...
fragment half4 borderStrokeFragment(
    ...
    // fwidth(edgeDist) converts style width from "pixels" to barycentric units,
    // so borderWidth is truly data-driven (not a fixed hardware line width).
    float edgeDist = min(bary.x, min(bary.y, bary.z));
    float pixelToBary = max(fwidth(edgeDist), in.borderFwidthEpsilon);
