...
public final class PointCloudOIRPipeline: OverlayEncodable {
    ...
    private func buildPipelineStates() throws {
        ...
        // Render sparse dense-map evidence as circular point sprites.
        // The overlay should read like confidence dots, not quad/tile fragments.
        if let vertexFn = library.makeFunction(name: "overlayVertex"),
           let fragmentFn = library.makeFunction(name: "overlayFragment") {
            ...
            desc.colorAttachments[0].pixelFormat = .bgra8Unorm
            desc.colorAttachments[0].isBlendingEnabled = true
            // Standard premultiplied alpha for all channels; green should remain visible.
    ...
    public func encode(
        ...
        if pcCount == 0 && splatCount == 0 && overlayCount == 0 {
        ...
        // ─── Pass 1: Point Cloud (primary live capture visualization) ───
        // Subject-first capture now uses the pointmap/dense-map trunk as the
        // only active geometry visualization path. If point-cloud vertices are
        // present, they own the screen and the legacy overlay path stays idle.
        let renderPointCloudPass = pcCount > 0
        if renderPointCloudPass, pcCount > 0, let pso = pointCloudPSO {
        ...
        // ─── Pass 1.5: Quality Overlay (legacy fallback only) ───
        // Keep this path for non-pointmap fallbacks, but do not mix it with the
        // active point-cloud main path during subject-first capture.
        ...
        if let ovPSO = overlayPSO, overlayCount > 0, !renderPointCloudPass {
            ...
            if let encoder = commandBuffer.makeRenderCommandEncoder(descriptor: overlayRPD) {
                ...
                // Depth test ON so dots behind closer evidence stay hidden.
                encoder.setDepthStencilState(depthWriteState)

                encoder.setVertexBuffer(overlayBuffers[slot], offset: 0, index: 0)
                encoder.setVertexBuffer(pcUniformBuffers[slot], offset: 0, index: 1)
                encoder.drawPrimitives(type: .point, vertexStart: 0, vertexCount: overlayCount)
                encoder.endEncoding()
