...
final class ScanGuidanceIntegrationTests: XCTestCase {
    ...
    func testWedgeGeometryLOD2HasVolume() {
        ...
        // v4 tent: LOD low = 3 base + 1 peak = 4 vertices
        XCTAssertGreaterThanOrEqual(result.vertices.count, 4, "LOD2 should generate at least one tent wedge")
        XCTAssertEqual(result.vertices.count % 4, 0, "LOD2 should output 4 vertices per tent")
    ...
    func testWedgeGeometryLOD1HasMoreVertices() {
        ...
        // v4 tent shapes: all LODs produce more vertices than flat (3)
        XCTAssertGreaterThan(lod1.vertices.count, lod2.vertices.count,
                             "LOD1 (medium) should have more vertices than LOD2 (low)")
        // Full LOD uses shared vertices (7) vs medium's per-face normals (9),
        // so full may have fewer vertices but higher quality normals.
        XCTAssertGreaterThan(lod0.vertices.count, 3,
                             "LOD0 should have more than flat triangle vertices")
